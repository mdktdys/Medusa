...
async def create_state(session: AsyncSession = Depends(local_db_helper.session_dependency)) -> CreateStateDto:
    return await crud.create_state(session = session)


@router.post("/auth_status")
async def auth_status(request: AuthStatusRequest, session: AsyncSession = Depends(local_db_helper.session_dependency)) -> AuthStatusDto | None:
