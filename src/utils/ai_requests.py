...
extract_groups_promt: str = 'Напиши списком через запятую группы которые на практике. Учти, что запись, например групп, 21СА-1,2 это имеется ввиду 21СА-1 и 21СА-2 и написать такие группы надо отдельно. Не пиши ничего лишнего кроме групп'
