...
def get_group_default_schedule_formatted(group_id: int, week_number: int) -> str:
    ...
    for day_number in range(0, 6):
        ...
        for para in day_paras:
            result = (
                result
                + f"{para.number_para}|{get_course_by_id(para.course_id).name}|{get_teacher_by_id(para.teacher_id).name}\n"
            )
...
def get_group_default_schedule(group_id: int, week_number: int) -> list[Paras]:
    ...
    response: List[dict] = (
        supabase_connect.table("Paras")
        .select("*")
        .eq("group", group_id)
        .gte("date", start_week_date_moday)
        .lte("date", start_week_date_sundy)
        .execute()
        .data
    )
    paras: List[Paras] = []
...
def get_group_default_schedule_json(
    group_id: int, week_number: int
) -> dict[int, List[dict]]:
    paras = get_group_default_schedule(group_id=group_id, week_number=week_number)
    schudule = {}
    for day_number in range(0, 6):
        schudule[day_number + 1] = [
            x.toDict() for x in paras if x.date.weekday() == day_number
        ]
    return schudule
