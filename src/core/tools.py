...
weekdays_names = {1:"Понедельник",2:"Вторник",3:"Среда",4:"Четверг",5:"Пятница",6:"Суббота"}


def get_week_start_end(week_number: int, year: int) -> Tuple[datetime.date, datetime.date]:
    ...
    week_start = first_day_of_year - datetime.timedelta(days=days_difference) + datetime.timedelta(
        weeks=week_number - 1)
...
def get_teacher_by_id(teacher_id: int) -> Course:
    return [teacher for teacher in database_data.TEACHERS if teacher.id == teacher_id][0]
